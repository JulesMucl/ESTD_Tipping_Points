...
config = {
    ...
    "selection_resources": ["AMMONIA_RE", "H2_RE", "GAS_RE","GAS"],  # Ressources à sélectionner pour les graphiques
    ...
    "root_prefix": "GAS_RE_only",  # Préfixe du dossier racine
    "save_path": Path("C:/Users/julem/Dropbox/EPL/TFE/TFE_textes/Images/TIPPING_POINT/"),
    "cost_enabled": True,
    "use_plotly": False,

    # Préfixes et plages de scénarios
    "scenario_prefix_zoom": "GAS_RE_gwp_",
    "scenario_prefix_normal": "H2_RE_gwp_",
    "ZOOM" : False,  # Si True, utilise les scénarios zoom
    "scenario_range_zoom": range(20, 36, 2),
    "scenario_range_normal": range(0, 52, 2),
...
def plot_selected_resources(df_resources, selected_resources, save_as=None):
    ...
    for res in selected_resources:
        if res in df_resources.columns:
            plt.plot(df_resources.index, df_resources[res], label=res, marker='o', linestyle='-', linewidth=2,
...
def plot_installed_technos(df_assets, techs_to_plot, save_as=None):
    ...
    for tech in techs_to_plot:
        if tech in df_assets.columns:
            plt.plot(df_assets.index, df_assets[tech], label=tech, marker='o',
...
def plot_combined_figure(df_assets, df_resources, cost_dict, selection_technos, selection_resources, save_as=None):
    ...
    for tech in selection_technos:
        if tech in df_assets.columns:
            ax1.plot(df_assets.index, df_assets[tech], label=tech, marker='o', linestyle='-', linewidth=2, alpha=0.7)
        else:
            print(f"[⚠] Technology '{tech}' not found in data.")
...
def main():
    ...
    if ZOOM:
        scenarios = [f"{config['scenario_prefix_zoom']}{i/1000:.3f}" for i in config["scenario_range_zoom"]]
        root = Path(f"C:/Users/julem/EnergyScope_Original/ESTD_Original/case_studies/ONLY/{config['root_prefix']}/ZOOM")
    else:
        scenarios = [f"{config['scenario_prefix_normal']}{i/100:.3f}" for i in config["scenario_range_normal"]]
        root = Path(f"C:/Users/julem/EnergyScope_Original/ESTD_Original/case_studies/ONLY/{config['root_prefix']}")

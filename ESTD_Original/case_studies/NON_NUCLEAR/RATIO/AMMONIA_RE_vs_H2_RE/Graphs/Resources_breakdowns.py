...
# === 📁 RACINE
root_prefix = "AMMONIA_RE_vs_H2_RE"
root = Path(f"C:/Users/julem/EnergyScope_Original/ESTD_Original/case_studies/RATIO/{root_prefix}")

# === ❌ Ressources à exclure de tous les graphes
excluded_resources = ["CO2_EMISSIONS", "CO2_ATM", "CO2_INDUSTRY", "CO2_CAPTURED"]


# === 🔍 Recherche récursive sur deux niveaux
scenarios = sorted([
    (subfolder.relative_to(root)).as_posix()
    for folder in root.iterdir() if folder.is_dir()
    for subfolder in folder.iterdir()
    if subfolder.is_dir() and (subfolder / "output" / "resources_breakdown.txt").exists()
])
...
def load_data(scenarios, root, cost_enabled=True):
    ...
    for scenario in scenarios:
        ...
        if fn_res.exists():
            ...
            df = df[~df.index.isin(excluded_resources)]  # ⛔ Exclure ressources CO2
...
def split_stable_resources(df, threshold_pct=1, min_abs=0.1, exclude=None):
    if exclude is None:
        ...
        exclude = [exclude]
    exclude = list(set(exclude + excluded_resources))  # Ajouter exclusions fixes
    max_usage = df.max()
    min_usage = df.min()

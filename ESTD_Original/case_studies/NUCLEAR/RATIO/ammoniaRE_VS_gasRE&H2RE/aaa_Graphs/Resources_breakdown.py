...
config = {
    ...
    "root_prefix": "ammoniaRE_VS_gasRE&H2RE",  # Préfixe du dossier racine

    "save_path": Path("C:/Users/julem/Dropbox/EPL/TFE/TFE_textes/Images/TIPPING_POINT/NUCLEAR/RATIO/ammoniaRE_VS_gasRE&H2RE/"),  # Chemin de sauvegarde des graphiques
    "cost_enabled": True,
    "use_plotly": False,
...
def extract_gwp_values_flexible(name):
    """
    Tente d'extraire deux floats depuis une chaîne contenant '__' et '_vs_'
    Peu importe ce qu'il y a après.
    """
    try:
        if "__" not in name or "_vs_" not in name:
            return float("inf"), float("inf")
        suffix = name.split("__")[1]  # ex: "0.000_vs_0.050_truc"
        parts = suffix.split("_vs_")
...
def main():
    ...
    root = Path(f"C:/Users/julem/EnergyScope_Original/ESTD_Original/case_studies/NUCLEAR/RATIO/{root_prefix}")
    ...
    scenarios.sort(key=lambda name: tuple(reversed(extract_gwp_values_flexible(name))))
    print("[✅] Tri des scénarios effectué :")
    for s in scenarios:
        print(s, "→", extract_gwp_values_flexible(s))



    print(f"[✅] {len(scenarios)} scénarios détectés dans {root_prefix}")
    print(f"[✅] {len(scenarios)} scénarios triés !")
